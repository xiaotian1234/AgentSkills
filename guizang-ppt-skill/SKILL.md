...
description: 仅当用户明确要求生成 HTML/网页形式的 PPT 或演示文稿时使用（例如“做成 HTML 类型的 PPT”“生成单文件 HTML 演示”“网页 PPT”“HTML slide deck”“HTML presentation”“horizontal swipe deck”）。普通的“做 PPT/PowerPoint”、未指定 HTML 输出的“杂志风 PPT/瑞士风 PPT”以及 .pptx 请求不要触发本技能，应交给常规 PPT 技能处理。生成内容为单文件 HTML 横向翻页 deck，含 WebGL 背景、演讲者视图、观众屏同步、讲稿备注、章节幕封、数据大字报、图片网格等模板。
---
...
只有当用户明确表达“输出格式是 HTML/网页”或直接使用 HTML 演示相关说法时，才启用本 Skill。以下表达可以触发：

- “做成 HTML 类型的 PPT”“生成网页 PPT”“输出单文件 HTML 演示文稿”
- “HTML slide deck”“HTML presentation”“web presentation”“horizontal swipe deck”
- 在已经明确 HTML 输出的前提下，再指定“电子杂志风 / 瑞士风 / Swiss Style”等视觉风格

以下请求不要触发本 Skill：

- 仅说“做一份 PPT / PowerPoint / 演示文稿”，但没有 HTML/网页输出要求
- 仅说“杂志风 PPT”“瑞士风 PPT”“Swiss Style PPT”，但没有 HTML/网页输出要求
- 明确要求 `.pptx`、PowerPoint 文件或可在 PowerPoint 中编辑的演示文稿

如果用户只描述了视觉风格而没有指定输出格式，先询问其是否需要 HTML 网页 PPT；未确认前不要启用本 Skill。
