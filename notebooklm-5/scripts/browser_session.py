...
class BrowserSession:
    ...
    def _initialize(self):
        ...
        print(f"[RUN] Creating session {self.id}...")

        # Create new page (tab) in context
        self.page = self.context.new_page()
        print(f"  [WEB] Navigating to NotebookLM...")
        ...
        try:
            ...
            print(f"[X] Failed to initialize session: {e}")
            if self.page:
                self.page.close()
            raise
    ...
    def reset(self):
        ...
        print(f"[REFRESH] Resetting session {self.id}...")
