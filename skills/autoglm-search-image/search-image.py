#!/usr/bin/env python3
# search-image.py — AutoGLM 搜图
# 用法: python search-image.py "搜索关键词"
...
URL = "https://autoglm-api.zhipuai.cn/agentdr/v1/assistant/skills/search-image"
...
def main():
    if len(sys.argv) < 2 or not sys.argv[1].strip():
        print('用法: python search-image.py "搜索关键词"')
        sys.exit(1)

    query = sys.argv[1].strip()
    response = search_image(query)
